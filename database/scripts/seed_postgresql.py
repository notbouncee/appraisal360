...
def _load_env_from_repo_root() -> None:
    ...
    for candidate in (repo_root / "database" / ".env", repo_root / ".env"):
...
def _upsert_profiles(conn: psycopg.Connection) -> dict[str, uuid.UUID]:
    ...
    upsert_sql = """
    INSERT INTO public.profiles (user_id, display_name, team, role, must_change_password)
    ...
    with conn.cursor() as cur:
        for user in SEED_USERS:
            ...
            cur.execute(upsert_sql, (user_uuid, user.display_name, user.team))
