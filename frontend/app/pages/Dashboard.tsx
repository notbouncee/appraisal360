...
import { getFeedbackCount, getFeedbackReceived, getUpvoteCount, getUpvotesReceived, listProfiles } from "@/services/backendApi";
...
const Dashboard = () => {
  ...
  const { data: upvotesResult = { data: [], count: 0 } } = useQuery({
    ...
    queryFn: async () => {
      ...
      return getUpvotesReceived(profile.id, page, pageSize);
    },
    enabled: !!profile,
  });
  ...
  const { data: upvoteCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      return getUpvoteCount(profile!.id);
    },
    enabled: !!profile,
  });
  ...
  const { data: feedbackCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      return getFeedbackCount(profile!.id);
    },
    enabled: !!profile,
  });
  ...
  const { data: feedbackResult = { data: [], count: 0 } } = useQuery({
    ...
    queryFn: async () => {
      return getFeedbackReceived(profile!.id, feedbackPage, pageSize);
    },
